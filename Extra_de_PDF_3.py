# Nombre del scritp: Extra_de_PDF_3.py
...
# Función para extraer descripción, débito y saldo
def procesar_fila(texto):
    numeros = re.findall(r'-?\d{1,3}(?:\.\d{3})*,\d{2}-?', texto)
    if len(numeros) >= 2:
        pos = texto.find(numeros[0])
        descripcion = texto[:pos].strip()
        debito_str = numeros[0]
        saldo_str = numeros[1]

        # Limpieza y conversión a float
        debito = float(debito_str.replace('.', '').replace(',', '.').replace('-', '')) * (-1 if '-' in debito_str else 1)
        saldo = float(saldo_str.replace('.', '').replace(',', '.').replace('-', '')) * (-1 if '-' in saldo_str else 1)
...
# Aplicar a tu DataFrame ya cargado
...
df_ok = df_ok.drop(columns=['Column que debe quedar'])
...
print("Los datos extraídos se han guardado en 'registros_extraidos.xlsx'.")
print("En la carpeta: ", pdf_folder)
